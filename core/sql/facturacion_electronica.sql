...
CREATE DEFINER=`root`@`localhost` FUNCTION `f_nombre_mes`(`num_mes` INT)
	RETURNS varchar(30) CHARSET latin1
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
	SQL SECURITY DEFINER
	COMMENT 'devuelve nombre del mes'
BEGIN
if(num_mes = 1) THEN
	return "Enero";
ELSEIF(num_mes = 2) THEN
	return "Febrero";
ELSEIF(num_mes = 3) THEN
	return "Marzo";
ELSEIF(num_mes = 4) THEN
	return "Abril";
ELSEIF(num_mes = 5) THEN
	return "Mayo";
ELSEIF(num_mes = 6) THEN
	return "Junio";
ELSEIF(num_mes = 7) THEN
	return "Julio";
ELSEIF(num_mes = 8) THEN
	return "Agosto";
ELSEIF(num_mes = 9) THEN
	return "Septiembre";
ELSEIF(num_mes = 10) THEN
	return "Octubre";
ELSEIF(num_mes = 11) THEN
	return "Noviembre";									
ELSEIF(num_mes = 12) THEN
	return "Diciembre";										
ELSE
	return "";
END IF;
